...
namespace thruster_torque_and_differential_throttling
{
    ...
    class simplex_solver
    {
        ...
        {
            int tableau_size = width * height;

            if (tableau_size <= _current_size)
                Array.Clear(_tableau, 0, _current_size);
            else
            {
                _tableau      = new double[height, width];
                _current_size = tableau_size;
            }
